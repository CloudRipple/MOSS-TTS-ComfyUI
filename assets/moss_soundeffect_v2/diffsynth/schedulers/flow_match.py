# MOSS-TTS-V15-ComfyUI patch: inference-only trim. Removed the BSMNTW
# training-time timestep weighting (linear_timesteps_weights), training_weight()
# and training_target(); add_noise / step / set_timesteps keep upstream
# semantics. The `training` flag is retained because WanAudio pipeline units
# branch on `pipe.scheduler.training`.
import torch, math
...
class FlowMatchScheduler():
    ...
    def set_timesteps(self, num_inference_steps=100, denoising_strength=1.0, training=False, shift=None, dynamic_shift_len=None):
        ...
        # MOSS-TTS-V15-ComfyUI patch: upstream computed BSMNTW
        # linear_timesteps_weights here when training=True; that path is not
        # vendored. Only the flag is kept for pipeline-unit branching.
