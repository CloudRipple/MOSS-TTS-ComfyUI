...
class MossTTSDelayConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # MOSS-TTS-V15-ComfyUI patch: this composite config has no tied weights at
        # the top level (checkpoint stores untied emb_ext/lm_heads). On
        # transformers 4.x, `get_text_config(decoder=True)` returns *this* config
        # and PretrainedConfig defaults tie_word_embeddings=True, which makes
        # post_init() call the (argument-requiring) get_input_embeddings and
        # crash with TypeError. 5.x resolves the subconfig instead. Be explicit.
        self.tie_word_embeddings = False

        # MOSS-TTS-V15-ComfyUI patch: on transformers 4.x, PretrainedConfig.__init__
        # pops pad_token_id from kwargs (default None) and clobbers the value
        # assigned above; 5.x leaves it alone. Re-assert so both versions behave.
        self.pad_token_id = pad_token_id
