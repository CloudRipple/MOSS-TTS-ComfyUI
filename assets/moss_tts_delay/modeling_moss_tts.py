...
# MOSS-TTS-V15-ComfyUI patch: `transformers.initialization` only exists in
# transformers >= 5.0; the single usage below (init.normal_) is API-equivalent
# to torch.nn.init, which works on every transformers version.
...
class MossTTSDelayModel(MossTTSDelayPreTrainedModel):
    ...
    def generate(
        ...
        # MOSS-TTS-V15-ComfyUI patch: optional progress reporting, mirroring the
        # 1.7B Local-Transformer variant (callback(current_frame, frame_budget)).
        ...
        # MOSS-TTS-V15-ComfyUI patch: tqdm only when explicitly requested, plus
        # an optional progress_callback(current_frame, frame_budget).
        progress_bar = tqdm(
            range(max_new_tokens),
            desc=progress_description or f"Generating bs{batch_size} ...",
            disable=not show_progress,
        )
        if progress_callback is not None:
            progress_callback(0, max_new_tokens)
        for time_step in progress_bar:
            ...
            # MOSS-TTS-V15-ComfyUI patch: throttled progress callback.
            if progress_callback is not None and ((time_step + 1) % 8 == 0 or is_stopping.sum() == batch_size):
                progress_callback(time_step + 1, max_new_tokens)
