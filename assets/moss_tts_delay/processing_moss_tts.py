...
# MOSS-TTS-V15-ComfyUI patch: MODALITY_TO_BASE_CLASS_MAPPING only exists in
# transformers >= 5.0; on 4.x the equivalent registry is AUTO_TO_BASE_CLASS_MAPPING.
...
class MossTTSDelayProcessor(ProcessorMixin):
    # MOSS-TTS-V15-ComfyUI patch: transformers 4.x ProcessorMixin validates the
    # constructor kwargs against this class attribute (default is
    # ["feature_extractor", "tokenizer"]); 5.x infers attributes from the
    # __init__ signature and ignores it. Required for 4.x, harmless on 5.x.
    ...
    def encode_audios_from_path(
        self, wav_path_list: Union[str, "os.PathLike", List[Union[str, "os.PathLike"]]], n_vq: Optional[int] = None
    ):
        # MOSS-TTS-V15-ComfyUI patch: accept os.PathLike in addition to str.
